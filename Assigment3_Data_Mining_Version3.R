...
#*************************************************************
## GROUP 60 
## Cristina Buerbaum del R?o (2704514)
## Paola Sastre Dordelly (2716547)
## Andr?s Romero Herrera (2716340)
...
setwd(choose.dir(default = "", caption = "Select working directory folder"))
#setwd("C:/Users/Python/Documents/Master/Data_Mining/Assignment")

## 0.3 Download needed packages iff they are not installed yet  ####
pckg = c("readxl","dplyr","ggplot2","tidyverse", "stats", "lubridate",
         "Hmisc", "mice", "eeptools", "PerformanceAnalytics",
         "corrplot", "caret", "RWeka", "randomForest", "gridExtra", "naniar",
         "data.table", "Amelia", "scales", "ca", "polycor", "ltm", "mice", 
         "VIM", "lattice", "stats", "psy") 
...
##Get Sample
df_train = fread('training_set_VU_DM.csv',nrows = 2500000,header = TRUE,na.strings = c('NULL'))
df_test = fread('test_set_VU_DM.csv', header = TRUE,na.strings = c('NULL'))
#write.csv(df,file='2500000trainsample.csv',row.names=FALSE)
...
ctrate=percent(round((ckhotels/2500000),4))
...
full$visitor_location_country_id<-as.factor(full$visitor_location_country_id)
...
names(t[which.max(t)])
sum(full$click_bool[full$prop_id==116942])
